...
jest.mock('../../../../utils/position', () => ({
  get_horizontal_center_of_els: jest.fn(() => 0),
  get_vertical_center_of_els: jest.fn(() => 0),
  offset: jest.fn(() => Promise.resolve({ left: 0, top: 0 }))
}));

describe('position function', () => {
  test('calculates position for speccer element relative to the target element', async () => {
    ...
    jest.mock('../../../../utils/css', () => ({
      pinSpace: jest.fn(() => 10) // Replace with the desired mock value
    }));

    // Call the position function with different areas
    const leftPosition = await position(null, targetElement, speccerElement);
    const rightPosition = await position(SpeccerAreaEnum.Right, targetElement, speccerElement);
    const topPosition = await position(SpeccerAreaEnum.Top, targetElement, speccerElement);
    const bottomPosition = await position(SpeccerAreaEnum.Bottom, targetElement, speccerElement);
