...
describe('getContentForPin', () => {
  ...
  it('should return the symbol if the pin options are not provided', () => {
    ...
    assert.strictEqual(getContentForPin(symbol, targetElement, options),symbol);
  });

  it('should return the symbol if text option is false', () => {
    const options: SpeccerOptionsInterface = { pin: { text: false } } as SpeccerOptionsInterface;

    assert.strictEqual(getContentForPin(symbol, targetElement, options),symbol);
  });

  it('should return the symbol if data-speccer-title is not set', () => {
    const options: SpeccerOptionsInterface = { pin: { text: true } } as SpeccerOptionsInterface;
  ...
  it('should return a title span if data-speccer-title is set and no description is provided', () => {
    ...
    assert.strictEqual(getContentForPin(symbol, targetElement, options),
